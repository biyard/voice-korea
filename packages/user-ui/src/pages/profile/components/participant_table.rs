...
pub fn TableRow(lang: Language, project: Deliberation) -> Element {
    ...
    rsx! {
        div {
            class: "cursor-pointer flex flex-row w-full min-h-[55px] bg-white border-b border-b-[#e6e6e6] font-normal text-[15px] text-[#222222] max-[500px]:w-max",
            onclick: move |_| {
            ...
            div { class: "flex flex-1 px-[24px] py-[17px] gap-[10px] font-semibold overflow-hidden text-ellipsis whitespace-nowrap max-[500px]:w-200 max-[500px]:flex-none",
                div { {icon} }
                "{project.title}"
            }
