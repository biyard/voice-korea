...
test('Google OAuth Login and Save Session', async () => {
    ...
    await popup.screenshot({ path: 'screenshots/users/google-001/03-google-popup.png' });
    ...
    if (await popup.isVisible('input[type="email"]')) {
        ...
        await popup.screenshot({ path: 'screenshots/users/google-001/04-enter-email.png' });
        ...
        await popup.waitForSelector('input[type="password"]', { timeout: timeouts.wait });
        await popup.fill('input[type="password"]', credentials.pass);
        await popup.click('button:has-text("Next")');
        await popup.screenshot({ path: 'screenshots/users/google-001/05-enter-password.png' });
